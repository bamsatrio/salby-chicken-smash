...
def orderView(request):
    if request.user.is_authenticated:
        ...
        if len(orderHistory) == 0:
            messages.info(request, "Kamu belum memesan.")
...
def checkout(request):
    if request.method == "POST":
        ...
        order.save()
        update = OrderUpdate(order_id=order.order_id, update_desc="Pesananmu sudah disimpan")
        update.save()
        thank = True
        id = order.order_id
...
def handeLogin(request):
    if request.method == "POST":
        ...
        if user is not None:
            ...
            messages.warning(request, "Password tidak valid!")
            return HttpResponseRedirect(request.META.get('HTTP_REFERER'))
...
def handleSignUp(request):
    if request.method == "POST":
        ...
        try:
            ...
            messages.warning(request, " Username sudah ada yang punya. coba bikin Username lain.")
            return HttpResponseRedirect(request.META.get('HTTP_REFERER'))
            ...
            myuser.phone = phone
            myuser.save()
            messages.success(request, " Akunmu berhasil dibuat")
            return HttpResponseRedirect(request.META.get('HTTP_REFERER'))
    else:
        return HttpResponse("404 - Not found")
...
def handlerequest(request):
    ...
    if verify:
        if response_dict['RESPCODE'] == '01':
            ...
            print('pemesanan tidak berhasil karena' + response_dict['RESPMSG'])
